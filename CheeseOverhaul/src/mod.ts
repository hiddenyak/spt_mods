    ...
    postDBLoad(container: DependencyContainer): void 
    {
        ...
        function tweakItems() 
        {
            ...
            for (const id in items) 
            {
                ...
                if (items[id]._type == "Item" && items[id]._props.DiscardLimit !== undefined) 
                {
                    items[id]._props["DiscardLimit"] = -1;
                }
                //Allow placing any item in secure container
                if (items[id]._parent == "5448bf274bdc2dfc2f8b456a" && items[id]._props.Grids[0]._props.filters !== undefined) 
                {
                    items[id]._props.Grids[0]._props.filters = [];
                }
                //Allow placing any item in backpack
                if (items[id]._parent == "5448e53e4bdc2d60728b4567" && items[id]._props.Grids[0]._props.filters !== undefined) 
                {
                    items[id]._props.Grids[0]._props.filters = [];
                }
                ...
                //Change the weight
                if (items[id]._type !== "Node" && items[id]._type !== undefined && (items[id]._parent !== "557596e64bdc2dc2118b4571" || items[id]._parent !== "55d720f24bdc2d88028b456d")) 
                {
                    items[id]._props["Weight"] = Math.round(0.5 * items[id]._props.Weight * 100) / 100;
                }
        ...
        function tweakRaids() 
        {
            ...
            for (const i in locations) 
            {
                for (const x in locations[i].base.exits) 
                {
                    ...
                    {
                        locations[i].base.exits[x].ExfiltrationTime = 20;
                    }
                }
            }
        }
        ...
        logger.logWithColor("~~~~ CHEESE OVERHAUL LOADED ~~~~", LogTextColor.BLACK, LogBackgroundColor.YELLOW);
    }
}
